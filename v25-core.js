// ===== Trail Coach v2.5.1 runtime patch =====
...
try{
  const p=document.querySelector(".version-pill");if(p)p.textContent="v2.5.1";
  const vb=document.querySelector(".version-big");if(vb)vb.textContent="v2.5.1";
  if($("appVersion"))$("appVersion").textContent="2.5.1";
  if($("appBuild"))$("appBuild").textContent="2026-09-06";
  const about=document.querySelector(".about-card .notice");
  if(about)about.textContent="v2.5.1 · flexibel Gym A/B/C + yoga, tillfällig omprioritering och separata VO₂-referenser · planner hotfix.";
}catch{}
